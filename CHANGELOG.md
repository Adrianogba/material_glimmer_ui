...
`GlimmerRefreshIndicator.workingFloor` is gone. The refresh no longer breathes
up from a floor while it works, so the constant stopped describing anything.

```dart
// Before: the bottom of a pulse that ran between it and full.
const floor = GlimmerRefreshIndicator.workingFloor; // 0.55

// After: the level the edge eases to and holds.
const level = GlimmerRefreshIndicator.workingLevel; // 0.7
```

If you never named that constant, nothing here needs changing on your side.

### Changed

- **The resting edge on a light ground.** A light card read as a flat rectangle
  with a border where the dark one read as a raised pane. Measured in luminance
  against its own surface, the dark ring is a highlight arc, +0.491 at the
  top-left and near nothing elsewhere. The light ring was an even outline in
  shade, 0.000, -0.201, -0.336, -0.241, with only 0.14 between its lightest and
  darkest visible stop.

  Inverting the dark ring is not the fix: that puts the bright stop at the
  bottom-right, and a form lit from below reads as pressed into the page. Light
  comes from the top-left in both themes. What differs is which direction has
  room, and a white highlight on a white surface is zero by definition. The
  light ring now says the same thing in shade, with a swing of 0.461 against
  dark's 0.495. The spread was the missing part, not the depth.

- **`GlimmerRefreshIndicator` holds one continuous value.** Releasing a pull
  used to drop the edge to a fixed floor in a single frame and then saw between
  that floor and full. It now eases into the hold over 260 ms, and a tapered
  highlight travels the lit edge while the work runs, the same shape
  `GlimmerProgressBar` and `GlimmerCircularProgress` already use.

- **`GlimmerProgressBar`'s indeterminate highlight glows.** It was drawn inside
  the track's clip, so it could soften but never leave. The bloom is drawn
  before the clip now, taller than the bar, and spills past it.

### Added

- `GlimmerRefreshIndicator.workingLevel`, `travelDuration` and
  `settleIntoWork`, for the hold, the travelling highlight and the ease into
  the hold.

### Example

- The gallery no longer opens on a Glass section. Frosted cards over a vivid
  gradient is a pass-through mock, which is a display-glasses idea, and a phone
  has nothing behind the screen. It opens on cards, actions, a stack and a list.
- Foundations drops Type and Depth, and gains an Edge section showing a resting
  and a focused surface side by side.
- The "Immersive controls" switch dimmed nothing. It is the ambient sweep now,
  and drives the card directly beneath it.
